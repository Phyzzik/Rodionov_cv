...
import time
import threading
import keyboard  # Добавлено для надежного перехвата ESC
...
EM_LEFT = 0.08                          # Левая граница аварийной зоны (как вы и просили)
...
with MSS() as sct:
    while True:
        ...
        if game_field_bbox is not None:
            ...
            # ВАЖНО: Маскируем внешние кнопки сайта (Anonym, Sound), затираем их белым цветом в frame_full
            # Кнопки находятся под динозавром (ниже его головы), очищаем этот участок на бинарном кадре
            if trex_bbox is not None:
                x_b, y_b, w_b, h_b = trex_bbox
                # Очищаем область под динозавром от постороннего UI сайта
                frame_full[y_b + h_b:, x1:x2] = 255 
            ...
            # Прыжок (используем 'space' вместо 'up')
            ...
            if (main_trigger or emerg_trigger) and (current_time - last_jump_time > JUMP_COOLDOWN):
                threading.Thread(target=pyautogui.press, args=('space',), daemon=True).start()
                last_jump_time = current_time

            cv2.imshow("Game Field", game_field)

        # Выход по ESC (проверка через OpenCV и системную библиотеку keyboard)
